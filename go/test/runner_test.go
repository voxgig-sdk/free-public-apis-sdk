...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FREE_PUBLIC_APIS_TEST_LIVE") == "TRUE" ||
		os.Getenv("FREE_PUBLIC_APIS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FREE_PUBLIC_APIS_TEST_EXPLAIN"); explain != "" {
		m["FREE_PUBLIC_APIS_TEST_EXPLAIN"] = explain
	}
