# free-public-apis-mcp

[MCP](https://modelcontextprotocol.io) server exposing the FreePublicApis SDK as
two agent tools — `free-public-apis_list` and `free-public-apis_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-public-apis-mcp)
make build
...
claude mcp add --scope user free-public-apis \
  -- /absolute/path/to/free-public-apis-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-public-apis-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-public-apis_list: first page of records
{ "entity": "ap_i" }
{ "entity": "ap_i", "query": { } }

// free-public-apis_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-public-apis-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-public-apis \
     -- "$PWD"/dist/*/free-public-apis-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-public-apis_list` and `free-public-apis_load` tools now appear
   in new sessions. Ask the agent to *"list ap_i using free-public-apis"*
   and it calls `free-public-apis_list` with `{"entity":"ap_i"}`.
...
### Call the `free-public-apis_list` tool
...
### Call the `free-public-apis_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `free-public-apis_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-public-apis_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_PUBLIC_APIS_APIKEY` | API key sent with every request. |
| `FREE_PUBLIC_APIS_BASE` | Optional override of the API base URL. |
...
```sh
./free-public-apis-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-public-apis_load","arguments":{"entity":"ap_i","query":{"id":1}}}}'
```
