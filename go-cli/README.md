...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-public-apis-cli)
make build

# 2. See usage (words, entities, env vars)
./free-public-apis-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-public-apis-cli
   ```
...
Run with no arguments to open a REPL (prompt `free-public-apis>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-public-apis-cli`. |
...
(`./free-public-apis-cli <expr>`) and interactively (the REPL), and expressions compose the
